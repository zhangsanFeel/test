...
class SecondHouse extends Base
{
    ...
    {	 
        //根据条件搜索
    	 $fields = array('city_id','price','proportion','room','orientation','purpose','decorate','prop_type',"search");
        ...
        if(!in_array($fitervalue['city_id'],$cityData) && !empty($fitervalue['city_id'])){
         ...
         //获得所有小区下的数据
        $secondes=model('second_house')->secondNorMalData($estateData,$where); 


        //获得数据总量
        $count=model('second_house')->secondCount($estateData,$where);


        $this->assign('count',$count);
        $this->assign('cityPrant_id',$cityPrant_id);
    	$this->assign('secondes',$secondes);
    ...
    public function main(){
        ...
        //图片信息
        $images=model('img')->getNormalImages($id,1);

        //数据信息
        $second=model('second_house')->get($id);

        //发布人信息
        $broker=model('broker')->get($second['broker_id']);
